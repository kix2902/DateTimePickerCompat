...
public class RadialPickerLayout extends FrameLayout implements OnTouchListener {
	...
	public void initialize(Context context, int initialHoursOfDay, int initialMinutes,
		...
		mHideAmPm = AccessibilityManagerCompat.isTouchExplorationEnabled(mAccessibilityManager) ? true
				: mIs24HourMode;
	 ...
	 *            be allowed to snap to whichever is closer. Use 1 to force strictly higher, -1 to
	 *            force
	 ...
	 * @param isInnerCircle
	 *            Whether the selection should be in the inner circle; will be ignored
	 *            if there is no inner circle.
	 * @param forceToVisibleValue
	 *            Even if the currently-showing circle allows for fine-grained
	 *            selection (i.e. minutes), force the selection to one of the visibly-showing
	 *            values.
	 * @param forceDrawDot
	 ...
	 *            The y coordinate.
	 * @param forceLegal
	 *            Force the selection to be legal, regardless of how far the coordinates are
	 *            from the actual numbers.
	 * @param isInnerCircle
	 *            If the selection may be in the inner circle, pass in a size-1 boolean
	 *            array here, inside which the value will be true if the selection is in the inner
	 *            circle,
		...
		switch (event.getAction()) {
			case MotionEvent.ACTION_DOWN:
				...
				if (mIsTouchingAmOrPm == AM || mIsTouchingAmOrPm == PM) {
					...
					boolean forceLegal = AccessibilityManagerCompat
